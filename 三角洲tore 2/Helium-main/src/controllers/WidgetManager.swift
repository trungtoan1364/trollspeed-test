...
class WidgetManager: ObservableObject {
    ...
    public func getWidgetSets() -> [WidgetSetStruct] {
        ...
        if let dict: [[String: Any]] = defaults.array(forKey: "widgetProperties", forPath: "") as? [[String: Any]] {
            for s in dict {
    ...
    public func saveWidgetSets() {
        ...
        if dict.count > 0 {
            defaults.setValue(dict, forKey: "widgetProperties", forPath: "")
        } else {
            // remove from the defaults
            defaults.removeObject(forKey: "widgetProperties", forPath: "")
        }

        DarwinNotificationCenter.default.post(name: "")
    }
